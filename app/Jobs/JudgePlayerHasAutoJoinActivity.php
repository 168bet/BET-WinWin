...
class JudgePlayerHasAutoJoinActivity implements ShouldQueue
{
    ...
    {
        $activities =  CarrierActivity::active()->autoApply()->get();
        \WLog::info('开始检测玩家'.$this->player->user_name.'是否可以自动参与活动');
        if($activities->count() > 0){
            $activities->each(function(CarrierActivity $activity){
                try{
                    ...
                    \WLog::error('该玩家'.$this->player->user_name.'不能自动参与该活动,活动名称:'.$activity->name.' 因为:'.$e->getMessage().' file:'.$e->getFile());
                }
            });
        }else{
