...
class CarrierActivity extends Model
{
    ...
    public static function createRules($current_carrier_id){
        return array_merge(self::$rules,[
            'name' => 'required|max:100|unique:inf_carrier_activity,name,NULL,id,carrier_id,'.$current_carrier_id,
        ]);
    }

    public static function updateRules($current_carrier_id,$except_id){
        return array_merge(self::$rules,[
            'name' => 'required|max:100|unique:inf_carrier_activity,name,'.$except_id.',id,carrier_id,'.$current_carrier_id,
        ]);
    }
