...
class CarrierPlayerLevel extends Model
{
    ...
    protected $appends = ['upgrade_rule_sql_string'];
    ...
    public function scopeByGamePlatId(Builder $query,$gamePlat){
        return $query->with(['rebateFinancialFlow' => function($builder) use ($gamePlat){
    ...
    public static function createRules($current_carrier_id){
        return array_merge(self::$rules,[
            ...
            'is_default' => 'required|boolean|unique:inf_carrier_player_level,is_default,NULL,id,is_default,1',
    ...
    public static function updateRules($current_carrier_id, $except_id){
        return array_merge(self::$rules,[
            ...
            'is_default' => 'required|boolean|unique:inf_carrier_player_level,is_default,'.$except_id.',id,is_default,1,carrier_id,'.$current_carrier_id,
    ...
    public static function cacheRebateFinancialFlow($playerLevelId,$gamePlatId){
        \WLog::info('获取洗码缓存配置KEY:'.CacheConstantPrefixDefine::CARRIER_PLAYER_LEVEL_REBATE_FINANCIAL_FLOW_CONFIGURE_CACHE_PREFIX.$playerLevelId.'_'.$gamePlatId);
