...
class Carrier extends Model
{
    ...
    protected static function boot(){
        ...
        self::updated(function(Carrier $carrier){
            ...
            //备用网站
            CarrierBackUpDomain::get(['domain'])->where('carrier_id',$carrier->id)->each(function(CarrierBackUpDomain $domain){
    ...
    public function checkIsAllowUserLogin(){
        ...
        if($this->dashLoginConf->is_allow_player_login == false){
            throw new CarrierAccountException('运营商禁止会员登陆');
        }
        return true;
    }
    ...
    public function checkIsAllowAgentLogin(){
        ...
        if($this->dashLoginConf->is_allow_agent_login == false){
            throw new AgentAccountException('运营商禁止代理登陆');
        }
        return true;
    }
    ...
    /**
     * 运营商额度是否足够
     * @param float $takenQuota 减去某个额度后
     * @return bool
     */
