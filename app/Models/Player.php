 ...
 * @property string $password 用户登陆密码
 ...
 * @property bool $user_status 用户状态:0 表示锁定(某段时间后可以重试登陆) 1表示正常 2表示关闭(用户不能再登陆)
...
class Player extends Auth
{
    ...
    protected static function boot(){
        ...
        //更新玩家信息时需要清空注册的缓存
        Player::updated(function(Player $player){
            PlayerGameAccount::byPlayerId($player->player_id)->each(function(PlayerGameAccount $gameAccount){
                \Cache::forget(CacheConstantPrefixDefine::PLAYER_GAME_ACCOUNT_INFO_PREFIX.$gameAccount->account_user_name);
            });
        });
    }
     ...
     * @var array
     */
    protected $hidden = ['password','pay_password','password_wrong_times','password_wrong_time'];
    ...
    public static function allPlayers(){
        return self::where('user_status',self::USER_STATUS_OK)->get(['player_id','user_name','real_name']);
    }
    ...
    public function scopeAgentNewMemberSum(Builder $query, $agent_id,$start_time,$end_time){
        return $query->where('agent_id', $agent_id)->whereBetween('created_at',[$start_time,$end_time]);
    }



    /**
     * 游戏账户账号, 运营商id +  代理商id + 用户账号
     * @return string
     */
    public function gameAccountUserName(){
        return $this->carrier_id.$this->user_name;
    }
    ...
    public static function onlineMeta(){
        return [ self::ONLINE_ON => '在线',self::ONLINE_OFF => '不在线'];
    }
    ...
    public static function userSex(){
        return [self::SEX_MAN => '男', self::SEX_WOMAN => '女'];
    }
    ...
    public static function userStatusMeta(){
        return [self::USER_STATUS_OK => '正常' , self::USER_STATUS_LOCKED => '锁定', self::USER_STATUS_CLOSED => '关闭'];
    }
