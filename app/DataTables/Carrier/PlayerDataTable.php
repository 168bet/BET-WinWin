...
class PlayerDataTable extends DataTable
{
    ...
    public function ajax()
    {
        $dataTables = $this->datatables
            ...
            ->addColumn('user_status_name',function(Player $player){
            ...
            ->addColumn('is_online_name',function(Player $player){
    ...
    public function query()
    {
        $players = Player::select([
            ...
            'login_ip',
            'recommend_player_id'
        ])->with(['agent','playerLevel','invitedPlayer']);
    ...
    private function columnEditRender($columnName){
        $columnEditScript = "$.fn.overlayToggle();  $(\\\"#editAddModal\\\").load(\\\"".route('players.showPlayerMainAccountAmountSettingModal')."\\\",{ player_id : player_id },function(){}).modal(\\\"show\\\")";
    ...
    private function getColumns()
    {
        ...
        return [
            '序号' => ['name' => 'user_name', 'render' => 'function(){ return null}','style' => 'text-align:center;width:40px'],
            '账号' => ['name' => 'user_name', 'data' => 'user_name','render' => 'function(){
            ...
            '姓名' => ['name' => 'real_name', 'data' => 'real_name','style' => 'text-align:center'],
            '会员等级' => ['name' => 'player_level_id', 'data' => 'player_level.level_name','defaultContent' => '','style' => 'text-align:center','searchable' => false],
            '邀请人' => ['name' => 'recommend_player_id','data' => 'invited_player', 'render' => 'function(){
                if(this.invited_player){
                    return this.invited_player.real_name;
                }
                return null;
            }','style' => 'text-align:center'],
            '所属代理' => ['name' => 'agent.realname', 'data' => 'agent.realname','render' => 'function(){
                if(this.agent){
                   return this.agent.is_default ? "系统用户" : this.agent.realname;
                }
                return null;
            }','style' => 'text-align:center'],
            '主账户余额' => ['name' => 'main_account_amount', 'data' => 'main_account_amount','style' => 'text-align:center','searchable' => false],
            '公司总输赢' => ['name' => 'total_win_loss', 'data' => 'total_win_loss', 'render' => 'function(){ 
            ...
            }','style' => 'text-align:center','searchable' => false],
            //'是否在线' => ['name' => 'is_online', 'data' => 'is_online_name','style' => 'display:hidden'],
            '最后登录时间' => ['name' => 'login_at','data' => 'login_at','style' => 'text-align:center;width:126px','searchable' => false],
//            '最后登录IP' => ['name' => 'login_ip', 'data' => 'login_ip', 'style' => 'text-align:center'],
            '访问网址' => ['name' => 'login_domain', 'data' => 'login_domain','style' => 'text-align:center;width:145px','searchable' => false],
//            '注册时间' => ['name' => 'created_at', 'data' => 'created_at','style' => 'text-align:center'],
            '备注' => ['name' => 'remark', 'data' => 'remark','style' => 'text-align:center','searchable' => false],
            '状态' => ['name' => 'user_status', 'data' => 'user_status_name','style' => 'text-align:center','searchable' => false]
        ];
    }
