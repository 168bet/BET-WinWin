...
class AgentAccountAdjustLogDataTable extends DataTable
{
    ...
    public function ajax()
    {
        $DataTables = $this->datatables
            ->eloquent($this->query())
            ->addColumn('adjust_type_string',function(AgentAccountAdjustLog $log){
                return AgentAccountAdjustLog::adjustTypeMeta()[$log->adjust_type];
            })
            ->addColumn('type_name',function(AgentAccountAdjustLog $log){
...
//        $agentAccountAdjustLogs = AgentAccountAdjustLog::with(['player.playerLevel','player.agent','operatorUser'])->orderBy('id','desc');
        $agentAccountAdjustLogs = AgentAccountAdjustLog::with(['agent.agentLevel','agent.parentAgent','operatorUser'])->orderBy('id','desc');
            ...
            　　    var startIndex= api.context[0]._iDisplayStart;
                ...
                　　});
                    ...
                    $(".user_edit").on("click",function(){
                        ...
                        $.fn.winwinAjax.buttonActionSendAjax(_me,"'.route('carrierAgentUsers.showAgentUserInfoEditModal',null).'/"+ user_id,{},function(content){
                            $.fn.overlayToggle();
    ...
    {
        return [
            '序号' => ['name' => 'id','data' => 'id','searchable' => false],
            '账号' => ['data' => 'agent.username','defaultContent' => '','render' => 'function(){
                return "<a class=\"text-primary user_edit\" data-id=\""+ this.agent.id +"\" style=\"cursor: pointer\">" + this.agent.username + "</a>"
            }'],
            '代理类型' => ['data' => 'type_name','defaultContent' => ''],
            '代理名称' => ['data' => 'agent.agent_level.level_name','defaultContent' => ''],
            '上级代理' => ['data' => 'agent.parent_agent.username','defaultContent' => ''],
            '调整类型' => ['name' => 'adjust_type', 'data' => 'adjust_type_string','searchable' => false],
            '金额' => ['name' => 'amount', 'data' => 'amount','searchable' => false],
            '调整时间' => ['name' => 'created_at','data' => 'created_at','searchable' => false],
            '备注' => ['name' => 'remark', 'data' => 'remark','searchable' => false],
            '操作人' => ['name' => 'operatorUser.username', 'data' => 'operator_user.username']
        ];
    }
