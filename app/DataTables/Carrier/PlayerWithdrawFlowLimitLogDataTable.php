...
class PlayerWithdrawFlowLimitLogDataTable extends DataTable
{
    ...
    public function ajax()
    {
        $dataTables = $this->datatables
            ...
            ->addColumn('selectCheckbox', function(PlayerWithdrawFlowLimitLog $log){
                return  "<input type=\"checkbox\" value='".$log->id."' class=\"square-blue log_check_box\">";
            })
            ->addColumn('limit_game_plats_name',function(PlayerWithdrawFlowLimitLog $log){
                if($log->limitGamePlats->count() > 0){
                    $game = $log->limitGamePlats->map(function(PlayerWithdrawFlowLimitLogGamePlat $element){
    ...
    {
        $playerWithdrawFlowLimitLogs = PlayerWithdrawFlowLimitLog::with(['player','carrierActivity','limitGamePlats.gamePlat'])->orderBy('created_at','desc');
    ...
    {
        return $this->builder()
            ->addColumn(['width' => '50px','title' => '<input style="display: none" type="checkbox" class="square-blue selectCheckbox">','data' => 'selectCheckbox'])
    ...
    {
        return [
            '账号' => ['name' => 'player.user_name', 'data' => 'player.user_name','defaultContent' => '','render' => 'function(){
                return "<a class=\"text-primary user_edit\" data-id=\""+ this.player.player_id +"\" style=\"cursor: pointer\">" + this.player.user_name + "</a>"
            }'],
            '限制来源' => ['name' => 'limit_type', 'data' => 'limit_type_string','defaultContent' => '','searchable' => false],
            '限游戏平台' => ['name','id','data' => 'limit_game_plats_name','defaultContent' => '','searchable' => false],
            '流水限额' => ['name' => 'limit_amount', 'data' => 'limit_amount','defaultContent' => '','searchable' => false],
            '已完成流水' => ['name' => 'complete_limit_amount', 'data' => 'complete_limit_amount','defaultContent' => '','searchable' => false],
            '是否已完成' => ['name' => 'is_finished', 'data' => 'is_finished','render' => 'function(){
                return this.is_finished ? \'是\' : \'否\';
            }','searchable' => false],
            '关联活动' => ['name' => 'related_activity','defaultContent' => '', 'render' => 'function(){
                return this.carrier_activity ? this.carrier_activity.name : "";
            }','searchable' => false
            ],
            '创建时间' => ['name' => 'created_at', 'data' => 'created_at','searchable' => false],
        ];
    }
