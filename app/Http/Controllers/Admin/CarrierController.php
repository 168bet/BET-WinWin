...
class CarrierController extends AppBaseController
{
    ...
    {
        $carrier = $this->carrierRepository->with('pins.defPin')->findWithoutFail($id);
        if (empty($carrier)) {
    ...
    public function edit($id)
    {
        ...
        return view('Admin.carriers.edit')->with('carrier', $carrier);
    }
    ...
    public function loginCarrierAdminSystem($carrierId){
        ...
        $carrierUser = CarrierUser::superAdmin()->where('carrier_id',$carrier->id)->first();
        if(!$carrierUser){
            $carrierUser = new CarrierUser();
