...
//        $carrierAgentLevelType = $repository->allAgentLevels($carrierid);//获取代理类型数据
        ...
        return view('Carrier.carrier_agent_users.create')->with('carrierAgentLevel', $carrierAgentLevel);
    }
    ...
    public function show($id)
    {
        ...
        return view('Carrier.carrier_agent_users.show')->with(['carrierAgentUser'=>$carrierAgentUser]);
    }
    ...
    public function showAgentUserInfoEditModal($id){
        ...
        {
            $carrierAgentLevel = \App\Models\CarrierAgentLevel::where(['id'=>$carrierAgentUser['agent_level_id']])->first();
        ...
        $carrierAgentDomain = \App\Models\CarrierAgentDomain::where(['agent_id'=>$carrierAgentUser['id']])->get();
        return view('Carrier.carrier_agent_users.agent_user_info_edit_modal')->with(['carrierAgentDomain'=>$carrierAgentDomain,'carrierAgentUser'=>$carrierAgentUser,'carrierAgentLevel'=>$carrierAgentLevel,'carrierAgentLevelType'=>$carrierAgentLevelType]);
    }
    ...
    public function agentSubPlayer($id)
    {
        ...
        $player = \App\Models\Player::where(['agent_id'=>$carrierAgentUser['id']])->get();
        return view('Carrier.carrier_agent_users.agent_sub_player',['player'=>$player]);
    }
    ...
    public function subAgent($id)
    {
        ...
        return view('Carrier.carrier_agent_users.sub_agent',['carrierSubAgentUser'=>$carrierSubAgentUser]);
    }
    ...
    public function agentAmount(Request $request)
    {
        ...
        $carrierAgentUser = $this->carrierAgentUserRepository->with(['carrier.carrierPayChannels' => function($query){
           $query->available();
        },'carrier.carrierPayChannels.payChannel.payChannelType'])->findWithoutFail($request->get('id'));
        if (empty($carrierAgentUser)) {
            return $this->renderNotFoundPage();
        }
   ...
   public function editPassword($id,Request $request)
    {
        ...
        return view('Carrier.carrier_agent_users.edit_password',['carrierAgentUser'=>$carrierAgentUser]);
    }
    ...
    public function updateRealName($id,Request $request){
        ...
        $this->carrierAgentUserRepository->update(['realname' => $request->get('realname')], $id);
        return $this->sendSuccessResponse();
    }
    ...
    public function updateTelephone($id, Request $request){
        $this->validate($request, [
            'mobile' => ['required','unique:inf_agent,mobile,'.$id.',id','regex:/^1[3-9]\d{9}$/'],
        ],[],[
            'mobile' => '手机号码'
        ]);
        $carrierAgentUser = $this->carrierAgentUserRepository->findWithoutFail($id);
        ...
        $this->carrierAgentUserRepository->update(['mobile' => $request->get('mobile')], $id);
        return $this->sendSuccessResponse();
    }
    ...
    public function updateEmail($id, Request $request){
        ...
        $this->carrierAgentUserRepository->update(['email' => $request->get('email')], $id);
        return $this->sendSuccessResponse();
    }
