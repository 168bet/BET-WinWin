...
class PlayerWithdrawLogController extends AppBaseController
{
    ...
    public function show($id)
    {
        ...
        return view('Carrier.player_withdraw_logs.show')->with('playerWithdrawLog', $playerWithdrawLog)->with('withDrawFlowRecords', $withDrawFlowRecords);
    }
    ...
    public function payModal($id){
        $playerWithdrawLog = $this->playerWithdrawLogRepository->with(['bankCard.bankType','player'])->findWithoutFail($id);
        if (empty($playerWithdrawLog)) {
            return $this->sendNotFoundResponse();
        }
        if($playerWithdrawLog->player){
            $playerTodayWithdrawCount = PlayerWithdrawLog::accountOut()->byPlayerId($playerWithdrawLog->player->player_id)->withdrawSucceedAtToday()->count();
        }else{
            $playerTodayWithdrawCount = 0;
        }
        return view('Carrier.player_withdraw_logs.payModal')->with(['playerWithdrawLog' => $playerWithdrawLog,'playerTodayWithdrawCount' => $playerTodayWithdrawCount]);
    }
        ...
        if($request->get('status') == PlayerWithdrawLog::STATUS_PAYED_OUT){
            ...
            switch ($feeUndertakenSide){
                ...
                case 'agent':
                    ...
                    break;
                case 'company':
                    $carrierConsumptionRecord->amount = $playerWithdrawLog->apply_amount + $feeAmount;
                    $input['finally_withdraw_amount'] = $playerWithdrawLog->apply_amount;
            }
...
//            }
            ...
            $carrierConsumptionRecord->consumption_source = '会员取款';
            $carrierConsumptionRecord->pay_channel_remain_amount = $carrierPayChannel->balance - $carrierConsumptionRecord->amount;
            ...
            try{
                ...
                \DB::transaction(function () use ($id,$carrierPayChannel,$input,$carrierConsumptionRecord,$playerWithdrawLog,$agentUndertakenRecord){
                    //2, 更新运营商资金流水记录
                    ...
                    $oldAmount = $playerWithdrawLog->player->main_account_amount + $playerWithdrawLog->player->frozen_main_account_amount;
                    $newAmount =  $oldAmount - $playerWithdrawLog->apply_amount;
                    $playerAccountLog->remark = '主账户原余额: '.$oldAmount.' 现余额: '.$newAmount.($input['fee_bear_side']== 'player' ? ' 手续费:'.$input['fee_amount'] : '');
                    //5,更新会员主账户余额
                    //6,玩家冻结余额减掉;
                    ...
                    //7删除取款之前已完成的流水;
                    PlayerWithdrawFlowLimitLog::byPlayerId($playerWithdrawLog->player->player_id)->finished()->where('created_at','<=',Carbon::now()->toDateString())->delete();
                });
