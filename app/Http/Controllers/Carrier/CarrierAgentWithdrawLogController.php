...
class CarrierAgentWithdrawLogController extends AppBaseController
{
        ...
        if($request->get('status') == CarrierAgentWithdrawLog::STATUS_PAYED_OUT){
            ...
            switch ($feeUndertakenSide){
                ...
                case 'agent':
                    ...
                    $agentUser->amount = $agentWithdrawLog->agent->amount -($agentWithdrawLog->apply_amount * ($feeAmount * 0.01));
                    $carrierConsumptionRecord->amount = $agentWithdrawLog->apply_amount;
                    $input['finally_withdraw_amount'] = $agentWithdrawLog->apply_amount;
                    break;
                case 'company':
                    $carrierConsumptionRecord->amount = $agentWithdrawLog->apply_amount + $feeAmount;
                    $input['finally_withdraw_amount'] = $agentWithdrawLog->apply_amount;
            }
            ...
            $carrierConsumptionRecord->consumption_source = '代理取款';
            ...
            try{
                ...
                \DB::transaction(function () use ($id,$carrierPayChannel,$input,$carrierConsumptionRecord,$agentWithdrawLog,$agentUser){
                    //2, 更新运营商资金流水记录
    ...
    public function payModal($id){
        $agentWithdrawLog = $this->carrierAgentWithdrawLogRepository->with(['bankCard.bankType','agent'])->findWithoutFail($id);
        if (empty($agentWithdrawLog)) {
            return $this->sendNotFoundResponse();
        }
    ...
    public function refuseWithdrawApply($id, Request $request)
    {
        ...
        try{
            ...
            \DB::transaction(function () use($input,$id,$agentWithdrawLog){
