...
class CarrierActivityAuditController extends AppBaseController
{
    ...
    {
        $carrierActivityAudit = $this->carrierActivityAuditRepository->with(['activity','player'])->findWithoutFail($id);
        if (empty($carrierActivityAudit)) {
        ...
        if($request->get('passed')){
            ...
            try{
                ...
                \DB::transaction(function () use ($carrierFactory,$player,$request,$carrierActivityAudit){
                    //处理活动的的红利数据
                    $carrierFactory->handleBonus($player);
                    //处理调整的红利数据
                    $adjustBonus = $request->get('adjust_is_plus') == 1 ? $request->get('amount') : (-1 * $request->get('amount'));
                    $withdrawLimit = $request->get('withdraw_limit_amount');
                    if($adjustBonus){
    ...
    public function bonusEdit($id){
        ...
        try{
            $carrierFactory = ActivityPassReviewBonusFactory::createFactory($carrierActivityAudit);
            list($bonus,$withdrawFlowLimit) = $carrierFactory->getBonusAndWithdrawLimitFlow(Player::findOrFail($player_id));
