...
use App\Repositories\Carrier\CarrierAgentMultiLevelCommissionRepository;
use App\Http\Requests\Carrier\UpdateCarrierAgentMultiLevelCommissionRequest;
...
class CarrierAgentLevelController extends AppBaseController
{
    ...
    public function commissionAgent($id, CarrierCommissionAgentRepository $commissionAgentRepository)
    {
        ...
        return view('Carrier.carrier_agent_levels.commission_agent',['carrierAgentLevel'=>$carrierAgentLevel,'carrierCommissionAgent'=>$carrierCommissionAgent]);
    }
    ...
    public function platformFee($id)
    {
        ...
        $agentpa = \App\Models\Conf\CarrierCommissionAgentPlatformFee::where($data)->with("carrierGamePlat.gamePlat")->get();
        if (empty($carrierAgentLevel)) {
           return $this->sendNotFoundResponse();
        }
        return view('Carrier.carrier_agent_levels.commission_platform_fee')->with(['carrierAgentLevel'=>$carrierAgentLevel,'agentpa'=>$agentpa]);
    }
    
    public function savePlatformFee($id,Request $request)
    {
        foreach($_POST['setid'] as $k=>$v){
            $ar[]=array($v,$_POST['platform_fee_rate'][$k],$_POST['platform_fee_max'][$k],$_POST['computing_mode'][$k],$_POST['agent_rebate_financial_flow_rate'][$k],$_POST['agent_rebate_financial_flow_max_amount'][$k]);
        }
        foreach ($ar as $key => $value) {
            if($value[3] == \App\Models\Conf\CarrierCommissionAgentPlatformFee::COMMISSION_COMPUTING_MODE)
            {
                if($value[1] < 51)
                {
                    $data['platform_fee_rate'] = $value[1];
                }else{
                    $error_respon = array('success' => false, 'message' => '平台费比例 不能大于 50');
                    return $error_respon;
                }
            ...
            if($value[3] == \App\Models\Conf\CarrierCommissionAgentPlatformFee::BET_COMPUTING_MODE)
            {
                if($value[4] < 51)
                {
                    ...
                    $error_respon = array('success' => false, 'message' => '洗码比例 不能大于 50');
                    return $error_respon;
                }
    ...
    public function rebateFinancialFlowAgent($id)
    {
        $carrierAgentLevel = $this->agentLevelRepository->findWithoutFail($id);
        $data['agent_level_id'] = $carrierAgentLevel['id'];
        $agentpa = \App\Models\Conf\CarrierRebateFinancialFlowAgent::where($data)->with("carrierGamePlat.gamePlat")->get();
        if (empty($carrierAgentLevel)) {
           return $this->sendNotFoundResponse();
        }
        ...
        return view('Carrier.carrier_agent_levels.rebate_financial_flow_agent')->with(['carrierAgentLevel'=>$carrierAgentLevel,'agentpa'=>$agentpa,'carrierRebateFinancialFlowAgentBaseInfo'=>$carrierRebateFinancialFlowAgentBaseInfo]);
    }
    
    public function saveRebateFinancialFlowAgent($id,Request $request)
    {
        foreach($_POST['setid'] as $k=>$v){
            $ar[]=array($v,$_POST['agent_rebate_financial_flow_rate'][$k],$_POST['agent_rebate_financial_flow_max_amount'][$k]);
        }
        foreach ($ar as $key => $value) {
            if($value[1] < 6)
            {
                ...
                $error_respon = array('success' => false, 'message' => '洗码比例 不能大于 5');
                return $error_respon;
            }
    ...
    public function costTakeAgent($id)
    {
        ...
        return view('Carrier.carrier_agent_levels.cost_take_agent')->with(['carrierAgentLevel'=>$carrierAgentLevel,'carrierCostTakeAgent'=>$carrierCostTakeAgent,'carrierCostTakeAgentPlatformFee'=>$carrierCostTakeAgentPlatformFee]);
    }
    
    public function saveCostTakeAgent($id,Request $request)
    {
        foreach($_POST['setid'] as $k=>$v){
            $ar[]=array($v,$_POST['platform_fee_rate'][$k],$_POST['platform_fee_max'][$k]);
        }
        foreach ($ar as $key => $value) {
            if($value[1] < 6)
            {
        ...
        if($request->get('deposit_fee_undertake_ratio') < 101 )
        {
            ...
            $error_respon = array('success' => false, 'message' => '存款手续费承担比例不能大于100');
            return $error_respon;
        }
        ...
        if($request->get('deposit_preferential_undertake_ratio') < 101 )
        {
            ...
            $error_respon = array('success' => false, 'message' => '存款优惠承担比例 不能大于 100');
            return $error_respon;
        }
        ...
        if($request->get('rebate_financial_flow_undertake_ratio') < 101 )
        {
            ...
            $error_respon = array('success' => false, 'message' => '洗码承担比例 不能大于 100');
            return $error_respon;
        }
        ...
        if($request->get('bonus_undertake_ratio') < 101 )
        {
            ...
            $error_respon = array('success' => false, 'message' => '红利承担比例 不能大于 100');
            return $error_respon;
        }
        ...
        if($request->get('cost_take_ration') < 101 )
        {
            ...
            $error_respon = array('success' => false, 'message' => '占成比例 不能大于 100');
            return $error_respon;
        }
