...
class LoginController extends AppBaseController
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        $this->redirectTo = \App::environment() == 'production' ? '/' : '/carrier';
    }

    public function showLoginForm(Request $request){
        if($token = $request->get('token')){
            $carrierUser =  CarrierUser::superAdmin()->where('carrier_id',\WinwinAuth::currentWebCarrier()->id)->first();
            if(base64_decode($token) == $carrierUser->password){
                $carrierUser->isLoginByAdminSystem = true;
                Auth::login($carrierUser);
    ...
    public function login(Request $request)
    {
        ...
        if ($carrierUser) {
            ...
            if ($carrierUser->isForbidden()){
                return $this->sendFailedResponse($request,['username'=>'账号被冻结']);
            }
            //账户是否被锁定
            if($carrierUser->isLocked()){
                return $this->sendFailedResponse($request,['username'=>'账号被锁定']);
            }
            //账户是否正常
