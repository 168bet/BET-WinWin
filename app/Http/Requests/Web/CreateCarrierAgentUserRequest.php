...
class CreateCarrierAgentUserRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if($agentRegisterConf){
            if((($agentRegisterConf->agent_realname_conf_status & CarrierDashLoginConf::IS_REQUIRED) == CarrierDashLoginConf::IS_REQUIRED) && ($agentRegisterConf->is_check_exist_agent_real_user_name == 1)){
                self::$rules = array_merge(self::$rules,[
                    'realname'=>'string|min:2|max:12|unique:inf_agent,realname|required',///^[\u4e00-\u9fa5]{2,40}$/  ^([\u4e00-\u9fa5]+|([a-zA-Z]+\s?)+)$
                ]);
            }elseif ((($agentRegisterConf->agent_realname_conf_status & CarrierDashLoginConf::IS_REQUIRED) == CarrierDashLoginConf::IS_REQUIRED) && ($agentRegisterConf->is_check_exist_agent_real_user_name == 0)){
                self::$rules = array_merge(self::$rules,[
                    'realname'=>'string|min:2|max:12|required',
                ]);
            }elseif ((($agentRegisterConf->agent_realname_conf_status & CarrierDashLoginConf::IS_REQUIRED) != CarrierDashLoginConf::IS_REQUIRED) && ($agentRegisterConf->is_check_exist_agent_real_user_name == 1)){
                self::$rules = array_merge(self::$rules,[
