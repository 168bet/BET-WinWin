...
class PayOrderRuntime
{
     ...
     */
    public function __construct(Player $player,CarrierPayChannel $payChannel, $amount,PlayerBankCard $playerBankCard = null)
    {
        $player->checkLocked();
        $this->player = $player;
    ...
    public function createOrder($depositTime = null, $depositType = null, $depositBankParam = null){
        ...
        if($depositBankParam){
            if($gateWay instanceof GuofubaoPayOrderGateway){
                $gateWay->applyCustomPayCondition(function(GuofubaoPayOrderGateway &$gateway) use ($depositBankParam){
                    $gateway->orderBankCode = $depositBankParam;
                });
            }
        }
        return $gateWay->createOrder($this->player,$this->carrierPayChannel,$this->amount, $this->playerBankCard,$depositTime,$depositType);
    }
